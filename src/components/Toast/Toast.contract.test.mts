...
test('Toast visuals use backdrop blur and smooth non-bouncy motion', () => {
    ...
    const toastStackBlock =
        overlays.match(
            /\.oui-toast-stack\n[\s\S]*?\.oui-toast-stack\[data-position\^="top"\]/,
        )?.[0] ?? '';
    const toastViewportBlock =
        overlays.match(/\.oui-toast-viewport\n[\s\S]*?\.oui-toast\n/)?.[0] ?? '';
    assert.match(toastStackBlock, /position: fixed[\s\S]*pointer-events: none/);
    assert.doesNotMatch(
        toastStackBlock,
        /background:|box-shadow:|-webkit-backdrop-filter:|backdrop-filter:/,
    );
    ...
    assert.match(
        overlays,
        /\.oui-toast\s+position: relative[\s\S]*?background: color-mix\(in srgb, var\(--oui-toast-tone-color, var\(--oui-info-base\)\) 5%, transparent\)/,
    );
    assert.match(overlays, /\.oui-toast\s+position: relative[\s\S]*?box-shadow: none/);
    assert.match(
        overlays,
        /\.oui-toast\[data-tone="success"\]\s+--oui-toast-tone-color: var\(--oui-success-base\)/,
    );
    assert.match(
        overlays,
        /\.oui-toast\[data-tone="danger"\]\s+--oui-toast-tone-color: var\(--oui-danger-base\)/,
    );
    assert.match(overlays, /\.oui-toast:hover \.oui-toast-progress\s+animation-play-state: paused/);
    assert.match(
        overlays,
        /\.oui-toast-viewport\[data-state="closing"\][\s\S]*animation: ouiToastOut/,
    );
    ...
    assert.doesNotMatch(
        overlays,
        /--oui-toast-tone-bg|border-color: var\(--oui-toast-custom-border-color|\.oui-toast\[data-clickable="true"\]:hover[\s\S]*?background:/,
    );
    assert.doesNotMatch(
        animations,
        /oui-toast-overshoot|scale\(1\.018\)|cubic-bezier\(\.18, 1\.34|opacity: 0\s+filter: blur\(3px\)/,
    );
});
