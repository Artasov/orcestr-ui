...
export function DataTable<T>({
    ...
    const attachScrollRoot = useCallback(
                ...
                >
                    <colgroup>
                        ...
                        {visibleColumns.map((column) => (
                            <col
                                ...
                                style={{
                                    width: formatCssSize(
                                        actualColumnWidths[column.key] ?? column.width,
                                    ),
                    ...
                    <thead>
                        <tr>
                            ...
                            {selectable ? (
                                <th
                                    className="oui-data-table-select-cell"
                                    role="columnheader"
                                    scope="col"
                                >
                                    <span className="oui-data-table-select-control">
                                        <Checkbox
                                            ...
                                            disabled={
                                                !onSelectedRowKeysChange || visibleKeys.length === 0
                                            }
                            ...
                            {visibleColumns.map((column) => {
                                const activeSort =
                                    sortItems.find((item) => item.key === column.key) ?? null;
                                return (
                                    <th
                                        key={column.key}
                                        role="columnheader"
                                        ...
                                        data-resizing={
                                            resizingColumnKey === column.key ? 'true' : undefined
                                        }
                                        style={{
                                            width: formatCssSize(
                                                actualColumnWidths[column.key] ?? column.width,
                                    ...
                                    >
                                        <span className="oui-data-table-header-cell">
                                            {column.sortable ? (
                                                ...
                                                >
                                                    <span className="oui-data-table-sort-label">
                                                        {column.title}
                                                    </span>
...
function DataTableMessageRow({ colSpan, children }: { colSpan: number; children: ReactNode }) {
    return (
        <tr role="row">
            <td role="gridcell" colSpan={colSpan}>
...
function tableMinimumWidth<T>({
    ...
    return columns.reduce((total, column) => {
        const width =
            columnWidths[column.key] ?? numericWidth(column.width) ?? column.minWidth ?? 120;
        return total + width;
    }, base);
}
